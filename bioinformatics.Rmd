...
# Write out quality plots - Could just combine with the aboce using a pwalk call?
test2 %>%
  group_by(fcid) %>%
  nest() %>%
...
test4 <- test3 %>%
  ...
  mutate(target_gene = case_when(
    str_detect(pcr_primers, "fwh") ~ "fwh",
    str_detect(pcr_primers, "EIF") ~ "EIF"
  ))  %>%
  mutate(phmm = case_when(
    str_detect(pcr_primers, "fwh") ~  "diagnostic_alignments/model/Bactrocera_COI.rds",
    str_detect(pcr_primers, "EIF") ~"diagnostic_alignments/model/Bactrocera_EIF3L.rds"
  ))%>%
  mutate(ref_db = case_when(
    str_detect(pcr_primers, "fwh") ~ "reference/COI_idtaxa.rds",
    str_detect(pcr_primers, "EIF") ~ "reference/EIF3L_idtaxa.rds"
  ))%>%
  mutate(blast_db = case_when(
    str_detect(pcr_primers, "fwh") ~ "reference/COI_hierarchial.fa",
    str_detect(pcr_primers, "EIF") ~ "reference/EIF3L_hierarchial.fa"
  ))%>%
  mutate(exp_length = case_when(
    str_detect(pcr_primers, "fwh") ~  205,
    str_detect(pcr_primers, "EIF") ~ 217
  )) %>%
  mutate(genetic_code = case_when(
    str_detect(pcr_primers, "fwh") ~  "SGC4",
    str_detect(pcr_primers, "EIF") ~ "SGC0"
  )) %>%
  mutate( coding=TRUE) %>%
  ungroup() %>%
