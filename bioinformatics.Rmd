...
seqtab_nochim_fwh <- seqtab_nochim[str_detect(rownames(seqtab_nochim), "_fwh_"),]
seqtab_nochim_eif <- seqtab_nochim[str_detect(rownames(seqtab_nochim), "_EIF_"),]

seqtab_cut_fwh <- seqtab_nochim_fwh[,nchar(colnames(seqtab_nochim_fwh)) %in% 200:210]
seqtab_cut_eif <- seqtab_nochim_eif[,nchar(colnames(seqtab_nochim_eif)) %in% 211:223]

# Align to phmm
fwh_model <- taxreturn::subset_model(readRDS("diagnostic_alignments/model/Bactrocera_COI.rds"),
                                 primers = c("GGDACWGGWTGAACWGTWTAYCCHCC", "GTRATWGCHCCIGCTAADACHGG"))

eif_model <- taxreturn::subset_model(readRDS("diagnostic_alignments/model/Bactrocera_EIF3L.rds"),
                                 primers = c("GATGCGYCGTTATGCYGATGC", "TTRAAYACTTCYARATCRCC"))


seqs_fwh <- DNAStringSet(colnames(seqtab_cut_fwh))
names(seqs_fwh) <- colnames(seqtab_cut_fwh)

seqs_eif <- DNAStringSet(colnames(seqtab_cut_eif))
names(seqs_eif) <- colnames(seqtab_cut_eif)

phmm_filt_fwh <- taxreturn::map_to_model(seqs_fwh, model = fwh_model, min_score = 100, min_length = 100, shave = FALSE, check_frame = TRUE, kmer_threshold = 0.5, k=5, extra = "fill")

phmm_filt_eif <- taxreturn::map_to_model(seqs_eif, model = eif_model, min_score = 100, min_length = 100, shave = FALSE, check_frame = TRUE, kmer_threshold = 0.5, k=5, extra = "fill")

#Filter sequences containing stop codons
codon_filt_fwh <- taxreturn::codon_filter(phmm_filt_fwh, genetic_code = "SGC4") # Invertebrate mito
codon_filt_eif <- taxreturn::codon_filter(phmm_filt_eif, genetic_code = "SGC0") # Standard genetic code

seqtab_final <- seqtab_nochim[,colnames(seqtab_nochim) %in% c(names(codon_filt_fwh), names(codon_filt_eif))]


message(paste0("Identified ",
               length(colnames(seqtab_nochim))  - length(colnames(seqtab_final)),
               " sequences containing stop codon out of ", length(colnames(seqtab_nochim)) , " input sequences."))
message(paste(sum(seqtab_final)/sum(seqtab_nochim),"of the abundance remaining after removing seqs with stop codons "))
...
cleanup <- st.all %>%
  ...
  mutate(type = case_when(
    ...
    !OTU %in% c(getSequences(seqtab_cut_fwh), getSequences(seqtab_cut_eif)) ~ "Incorrect size",
    !OTU %in% c(names(phmm_filt_fwh), names(phmm_filt_eif)) ~ "PHMM",
...
# Add ref_db column to the sample sheet
samdf <- read_csv("sample_data/Sample_info.csv")

samdf <- samdf %>%
  mutate(
  ref_db = case_when(
    str_detect(sample_id, "_fwh") ~  "reference/COI_idtaxa.rds",
    str_detect(sample_id, "_EIF") ~  "reference/EIF3L_idtaxa.rds"
  ),
  blast_db = case_when(
    str_detect(sample_id, "_fwh") ~  "reference/COI_hierarchial.fa",
    str_detect(sample_id, "_EIF") ~  "reference/EIF3L_hierarchial.fa"
  ))
  
write_csv(samdf, "output/logs/samdf_refs.csv")
  
# get list of all dbs
all_dbs <- unique(samdf$ref_db)
blast_dbs <- unique(samdf$blast_db)

# Also create a list to keep the classified taxonomy
taxtab_list <- vector("list", length=length(all_dbs))

# loop through each db
for (r in 1:length(all_dbs)){
  
  # Load the relevent db
  trainingSet <- readRDS(all_dbs[r])
  
  # get the samples that need to be classified with that db
  samples_to_subset <- samdf %>%
    filter(ref_db == all_dbs[r])
  
  # Subset the seqtab to just those samples
    seqtab_subset <- seqtab_final[str_remove(rownames(seqtab_final), pattern="_S[0-9].*$") %in%
                                      samples_to_subset$sample_id,]
  # only keep ASVs (seqtab columns) that are present in those samples (> 0 abundance)
  seqtab_subset <- seqtab_subset[,colSums(seqtab_subset) > 0]
  
  # get the sequences from the subset seqtab
  dna <- DNAStringSet(getSequences(seqtab_subset)) # Create a DNAStringSet from the ASVs
  ...
  db_name <- basename(all_dbs[r]) %>% str_remove("_.*$")
  
  # Output plot of ids for each db
	pdf(paste0("output/logs/", db_name,"idtaxa.pdf"), width = 11, height = 8 , paper="a4r")
	plot(ids)
	try(dev.off(), silent=TRUE)
  ...
  tax <- t(sapply(ids, function(x) {
    ...
    as.data.frame() %>%
    magrittr::set_rownames(getSequences(seqtab_subset)) %T>%
	write.csv(paste0("output/logs/", db_name,"idtaxa_results.csv")) %>%  #Write out logfile with confidence levels
    mutate_all(str_replace,pattern="(?:.(?!_))+$", replacement="") %>%
    magrittr::set_rownames(getSequences(seqtab_subset)) 
  
  # Top hit with BLAST
  seqs <- taxreturn::char2DNAbin(colnames(seqtab_subset))
  names(seqs) <- colnames(seqtab_subset)
  
  blast_spp <- blast_assign_species(query=seqs,db=blast_dbs[r], identity=97, coverage=95, evalue=1e06,
  ...
  saveRDS(blast_spp, paste0("output/rds/", db_name, "blast.rds"))
  ...
  saveRDS(tax_blast, paste0("output/rds/", db_name, "tax.rds"))
  
  # Also put the taxonomy table in the list so we dont need to load it in again
  taxtab_list[[r]] <- tax_blast
  
}

# Join the taxtabs back together
tax_final <- taxtab_list %>% 
  dplyr::bind_rows() %>%
  as.matrix()

# Write final taxonomy table to disk
saveRDS(tax_final, "output/rds/tax.rds") 


# Edit assignments
tax_final <- readRDS("output/rds/tax.rds")
...
final_tax <- readRDS("output/rds/tax.rds") 
...
test2 <- test  %>% 
  filter(str_detect(Species, "/"))  %>%
  dplyr::select(OTU, pcr_primers, rank_names(ps)) %>%
  distinct()
...
# On expected will need to merge:
#mayi/tenuifascias
#neo / tryoni
