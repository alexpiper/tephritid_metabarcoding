...
sinteractive --ntasks=1 --cpus-per-task=10 --mem-per-cpu=10GB --time=48:00:00
...
filt <- fs::dir_ls(path="diagnostic_alignments/merged", glob = "*.fa.gz")%>%
  purrr::map(function(x){
    ...
    pruned <- prune_groups2(purged, max_group_size = 5, discardby = "length", dedup = TRUE, prefer=prefered_taxa, quiet = FALSE)
    ...
    #  loci = loci_name,
    #  seqs = length(seqs),
    #  uniqseqs = length(uniqSeqs),
    #  namefilt = length(namefilt),
    #  phmm = length(filtered),
    #  codonfilt = length(codonfilt),
  ...
  for (d in 1:length(dataset)){
    ...
    if(dataset[d] == "dacine"){
      seqs <- seqs[str_detect(names(seqs) %>% 
                                str_remove("^.*;") %>%
                                str_remove(" .*$"), "Bactrocera|Dacus|Zeugodacus|Monacrostichus")]
    }else if (dataset[d] == "target"){
      seqs <- seqs[(names(seqs) %>% str_remove("^.*;")) %in% target_list$species]
    }else if (dataset[d] == "cue"){
      seqs <- seqs[(names(seqs) %>% str_remove("^.*;")) %in% cue_list]
    }else if (dataset[d] == "me"){
      seqs <- seqs[(names(seqs) %>% str_remove("^.*;")) %in% me_list]
    }
...
all_combinations <- id_combinations %>%
  ...
  group_modify(~{
    id_matrix <- .x %>%
      ...
      as.matrix()
    return(find_all_combinations(id_matrix, n=2))
  })


gg.all_bars <- all_combinations %>%
  dplyr::select(-where(is.list)) %>%
  group_by(win_length) %>%
  mutate(prop_id =  n_id / n_total) %>%
  ungroup() %>%
  mutate(comb = comb %>% 
           str_replace("EIF3L_EIF3L_", "EIF3L_")%>% 
           str_replace("DDOST_DDOST2_", "DDOST2_")%>% 
           str_replace("POP4_POP4_", "POP4_") %>% 
           str_replace("RPA2_RPA2_", "RPA2_") ) %>%
  mutate(comb = reorder_within(comb, -prop_id, within=win_length)) %>%
  mutate(win_length = win_length %>%
           factor(levels = c("full", "420bp", "220bp", "primer"))) %>%
  filter(!is.na(dataset)) %>%
  #mutate(n_loci = as.numeric(n_loci)) %>%
  ggplot(aes(x = comb, y = prop_id, fill=win_length))+
  geom_col() + 
  geom_text(aes(label=scales::percent(prop_id, accuracy=0.1)), vjust = 0.5, size=4, angle=90, hjust=1.2, colour="grey20")+
...
pdf(file= "fig/2primer_id_combinations.pdf", width = 11, height = 8, paper="a4r")
  plot(gg.all_bars)
try(dev.off(), silent=TRUE)
  
# Check differences between COI-EIF3L and COI-EIF3L-DDOST

setdiff(all_combinations %>%
  ungroup() %>%
  filter(dataset == "target", comb == "COI_fwhF2-fwhR2n-EIF3L_EIF3L_miniF4-EIF3L_miniR4") %>%
  unnest(failed) %>%
  pull(failed),
  all_combinations %>%
  ungroup() %>%
  filter(dataset == "target", comb == "COI_fwhF2-fwhR2n-DDOST_DDOST2_miniF1-DDOST2_miniR1") %>%
  unnest(failed) %>%
  pull(failed))
  
  
coi_id <- id_combinations %>% 
  filter(dataset == "target", loci == "COI_fwhF2-fwhR2n", mono=="correct") 

ddost_id <- id_combinations %>% 
  filter(!spp %in% coi_id$spp) %>% # Remove those already id'd by COI
  filter(dataset == "target", loci == "DDOST_DDOST2_miniF1-DDOST2_miniR1") %>%
  select(dataset, spp, ddost_id = nn) %>%
  left_join(
    id_combinations %>% 
    filter(dataset == "target", loci == "EIF3L_EIF3L_miniF1-EIF3L_miniR1") %>%
    select(dataset, spp, eifel_id = nn)
  )
...
mismatch_summaries <- mismatch %>% 
  mutate(species = template_name %>% str_remove("^.*;") %>% str_replace_all("_", " ")) %>%
...
dir.create("diagnostic_alignments/reference")

filt <- c("diagnostic_alignments/merged/COI_merged.fa.gz",
          "diagnostic_alignments/merged/EIF3L_merged.fa.gz") %>%
  purrr::map(function(x){
    loci_name <- basename(as.character(x)) %>% 
      str_remove("_.*$")
    ...
    # Filter out putatively mislabelled seqs
    misannot <- resolved[!names(resolved) %in% to_remove]
    ...
    # Align to phmm
    filtered <- taxreturn::map_to_model(resolved,  model, min_score = 100,
     shave=TRUE, trim_ends = 4, check_frame=TRUE,  max_N = 5, max_gap=(model$size/3), extra="drop", kmer_threshold = 0.3, k = 5,
     multithread=ncores, quiet=FALSE, progress = FALSE)
    ...
    purged  <- subset.DNAbin(codonfilt, subset = !str_replace(names(codonfilt), "(?:.(?!;))+$", "") %in% mixed_clusters$Acc)
 #
    ## prune over-represented groups - prefering inhouse sequences in case of ties
    #prefered_taxa <- fs::dir_ls("diagnostic_alignments/inhouse", glob=paste0("*",loci_name,"*")) %>%
    #  map(function(x){
    #    names(insect::readFASTA(x))
    #  })%>%
    ...
    insect::writeFASTA(purged, file=paste0("diagnostic_alignments/reference/", loci_name, ".fa.gz"), compress=TRUE)

    #Reformat to complete taxonomic heirarchy 
    #hierarchy <- reformat_hierarchy(pruned, db=db, quiet=FALSE)

	  #Write out results filtered results
    #insect::writeFASTA(hierarchy, file=paste0("diagnostic_alignments/reference/", loci_name, ".fa.gz"), compress=TRUE)
...
seqs <- insect::readFASTA("diagnostic_alignments/reference/EIF3L.fa.gz") %>%
  reformat_hierarchy(db = db)
...
seq_ref <- seqs
names(seq_ref) <- names(seqs) %>% str_replace_all("_", " ")
insect::writeFASTA(seq_ref, "reference/EIF3L_hierarchial.fa")
  
# Train IDTAXA
training_set <- train_idtaxa(
  seqs, max_group_size = 10, max_iterations = 3,  
  allow_group_removal = TRUE,  get_lineage = FALSE, quiet = FALSE
  )

#Write out training set
dir.create("reference")
saveRDS(training_set, file="reference/EIF3L_idtaxa.rds")

### TRAIN COI

seqs <- insect::readFASTA("diagnostic_alignments/reference/COI.fa") %>%
  reformat_hierarchy(db = db)
...
seq_ref <- seqs
names(seq_ref) <- names(seqs) %>% str_replace_all("_", " ")
insect::writeFASTA(seq_ref, "reference/COI_hierarchial.fa")
...
#Write out training set
dir.create("reference")
saveRDS(training_set, file="reference/COI_idtaxa.rds")
