...
filt <- fs::dir_ls(path="diagnostic_alignments/merged", glob = "*.fa.gz")%>%
  purrr::map(function(x){
    ...
    seqnames <- names(resolved) %>% str_remove("^.*;")
    names(resolved)[str_detect(seqnames, "sp\\.|spp\\.|aff\\.|nr\\.|bv\\.|cf\\.|nom\\.|nud\\.|environment|undescribed|unverified|unclassified|uncultured|unidentif|[0-9]|[:punct:]")] <- NA
    namefilt <- resolved[!is.na(names(resolved))]
    
    # Filter out putatively mislabelled seqs
    resolved <- resolved[!names(resolved) %in% to_remove]
    ...
    filtered <- taxreturn::map_to_model(namefilt,  model, min_score = 100,
    ...
    pruned <- taxreturn::prune_groups(purged, max_group_size = 5, discardby = "length", dedup = TRUE, quiet = FALSE)
...
# all
mismatch %>%
  mutate(genus = template %>% str_remove("^.*;") %>%
           str_remove(" .*$")) %>%
...
mismatch %>%
  ...
  filter(!is.na(mismatch)) %>%
  filter(species %in% target_list$species) %>%
  ggplot(aes(x = primer, y = species, fill=mismatch)) +
  geom_tile() +
  facet_grid(~direction, drop = TRUE, scales="free_x") +
  scale_x_reordered()+
...
for (p in 1:nrow(primers)){
  ...
  to_delete <- sample(seq(1, length(seqs),1), round(length(seqs)/5), replace = FALSE)
...
mismatch_summaries <- mismatch %>% 
  mutate(species = template %>% str_remove("^.*;")) %>%
