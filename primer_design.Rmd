...
gg.db_filt <- filt %>%
  ...
  ungroup()%>%
  mutate(stage = stage %>%
           str_replace("uniqseqs", "Unique Sequences") %>%
           str_replace("namefilt", "Binomial names only") %>%
           str_replace("misannot", "Misannotated taxonomy") %>%
           str_replace("phmm", "PHMM") %>%
           str_replace("codonfilt", "Codon filter") %>%
           str_replace("purged", "Mixed taxonomy") %>%
           str_replace("pruned", "Maximum 5 seqs per taxa")) %>%
  mutate(stage = factor(stage, levels=c("Unique Sequences",
                                        "Binomial names only",
                                        "Misannotated taxonomy",
                                        "PHMM",
                                        "Codon filter",
                                        "Mixed taxonomy",
                                        "Maximum 5 seqs per taxa"))) %>%
  ggplot(aes(x = stage, y = kept, fill=loci))+
  geom_col() +
  geom_text(aes(label=kept), colour="black",nudge_y = -50)+
  facet_grid(loci~., scales="free_y")+
  scale_fill_brewer(palette="Set1") +
    base_theme+
  theme(legend.position = "right")+
  labs(x = "Database curation step",
       y = "Sequences remaining",
       fill = "Locus")

gg.db_filt


pdf(file="fig/database_filt.pdf", width = 11, height = 8)
  plot(gg.db_filt)
...
filt <- c("diagnostic_alignments/cleaned/COI.fa.gz",
  ...
  purrr::map(function(x){
    ...
    # Find those with high intraspecific distances
    high_intra <- find_outlier_intraspp(filtered, threshold = 0.03)
    
    filtered2 <- filtered[!filtered %in% high_intra]
    
    insect::writeFASTA(filtered2, file=paste0("reference/", loci_name, ".fa.gz"), compress=TRUE)
    
    seqs_ref <- reformat_hierarchy(filtered2, db = db)
    
    #seqs_ref <- reformat_hierarchy(filtered, db = db)
    ...
    # Rename species complexes
    to_rename <- find_complexes(seqs_ref)
    
    # Make plots of barcode gap for the species complexes
    #    # Make full distance matrix
    #datd <- DECIPHER::DistanceMatrix(dna, includeTerminalGaps = FALSE, penalizeGapLetterMatches = TRUE,
    #                                 penalizeGapGapMatches = FALSE, correction = "Jukes-Cantor", processors = 1, verbose= TRUE)
    #
    #spp_names <- names(dna) %>%
    #  str_remove("^.*;")
    ...
    #sppVector = spp_names
    #
    ## Barcode gap - Do these for each species complex 
    #test <- tibble::enframe(intra_dist(datd, spp_names), name="Species", value="intra") %>%
    #  left_join(tibble::enframe(inter_dist(datd, spp_names), name="Species", value="inter"))
    #
    #test %>%
    ...
    new_names <- names(seqs_ref) %>%
      purrr::map_chr(~{
        current_spp <- .x %>% str_remove("^.*;")
        new_spp <- to_rename$new_names[match(current_spp, to_rename$old_names)]
        out <- .x %>% str_replace(current_spp, new_spp)
        return(out)
      })
    seqs_ref2 <- seqs_ref
    names(seqs_ref2) <- new_names
      
     # Write out refernce sets
   #names(seqs_ref2) <- names(seqs_ref2) %>% str_replace_all("_", " ")
    insect::writeFASTA(seqs_ref2, file=paste0("reference/", loci_name, "_hierarchial_renamed.fa.gz"), compress=TRUE)
    
    # Train IDTAXA
    training_set2 <- train_idtaxa(
      seqs_ref2, max_group_size = 10, max_iterations = 3,  
      allow_group_removal = TRUE,  get_lineage = FALSE, quiet = FALSE
      )
    
    #Write out training set
    dir.create("reference")
    saveRDS(training_set2, file=paste0("reference/", loci_name, "_idtaxa_renamed.rds"))
})%>%
  bind_rows()
